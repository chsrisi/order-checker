...
from ...models import User, RefreshToken, OutboundItem, PickItemEntry
...
def get_all_user_data() -> List[User]:
    with get_db() as db:
        return list(
            db.execute(select(User).filter(User.scope == "client")).scalars().all()
        )
...
def seed_admin_user(password_hash: str) -> bool:
    with get_db() as db:
        admin_user = (
            db.execute(select(User).filter(User.username == "admin")).scalars().first()
        )
        if not admin_user:
            logger.info("Admin user not found, seeding default admin account")
            db_admin = User(
                username="admin", password_hash=password_hash, scope="admin"
            )
            db.add(db_admin)
            db.commit()
            return True

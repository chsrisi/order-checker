...
from ...models import ShopeeOrder, ShopeeOrderInfo, ShopeeOrderRecipientAddress, ShopeeOrderItemList, BOMHeader
...
def get_shopee_orders_history() -> List[ShopeeOrder]:
    with get_db() as db:
        query = select(ShopeeOrder).order_by(ShopeeOrder.ship_by.desc()).options(
            selectinload(ShopeeOrder.info),
            selectinload(ShopeeOrder.item_list),
            selectinload(ShopeeOrder.recipient_address),
        )
